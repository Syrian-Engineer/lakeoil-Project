...
export default function StationCard({ station, setRefetchStation }: Props) {
  ...
  const operatorInfo = translate(stationCardTranslations,lang,"operatorInfo")
  const providerInfo = translate(stationCardTranslations,lang,"providerInfo")
  ...
  const handleEdit = () =>{ 
    route.push(`/station/edit-station?id=${station.id}`);
  }
  ...
  return (
    <div className={`relative bg-white border rounded-2xl shadow-sm hover:shadow-md transition p-6 w-full ${license.className} `}>
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex gap-3 text-gray-500 text-lg">
          <FaUserCog 
            className= {`w-6 h-6 hover:text-blue-600 cursor-pointer ${showOperator?"text-blue-600":""}`} 
            onClick={() => setShowOperator(p => !p)} 
          />

          <FaServer 
            className= {`w-6 h-6 hover:text-green-600 cursor-pointer ${showProvider?"text-green-600":""}`}
            onClick={() => setShowProvider(p => !p)} 
          />

          <IoIosPerson 
            className={`w-6 h-6 hover:text-purple-600 cursor-pointer ${showPersonInfo?"text-purple-600":""}`}
            onClick={() => setShowPersonInfo(p => !p)} 
          />
      ...
      {showOperator && (
        <div className={`mt-4 bg-blue-50 border border-blue-100 p-4 rounded text-sm`}>
          <h3 className="font-semibold text-blue-700 mb-2">{operatorInfo.text}</h3>
          <div className="space-y-1 text-blue-800">
            <p><strong>{t('name')}:</strong> {station.OperatorName}</p>
            <p><strong>TIN:</strong> {station.OperatorTin}</p>
            <p><strong>XTIN:</strong> {station.Operator_XTin}</p>
      ...
      {showProvider && (
        <div className="mt-4 bg-green-50 border border-green-100 p-4 rounded text-sm">
          <h3 className="font-semibold text-green-700 mb-2">{providerInfo.text}</h3>
          <div className="space-y-1 text-green-800">
            <p><strong>{t('provider')}:</strong> {station.VFD_provider}</p>
            <p><strong>{t('URL')}:</strong> {station.VFD_provider_URL}</p>
            <p><strong>{t('username')}:</strong> {station.VFD_provider_userName}</p>
